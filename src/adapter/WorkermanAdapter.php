...
/**
 * Workerman适配器 - Session 修复版本
 * 基于Workerman提供高性能HTTP服务器支持
 * 修复了 session_id 变化的问题，参考 ReactPHP adapter 实现
 *
 * 修复内容：
 * 1. 智能 Cookie 合并机制，保留 session 相关的 cookie
 * 2. 两种运行模式：优化模式和 ReactPHP 模式
 * 3. 详细的调试和监控功能
 * 4. 向后兼容原有配置
 */
class WorkermanAdapter extends AbstractRuntime implements AdapterInterface
{
    ...
    public function onMessage(TcpConnection $connection, Request $request): void
    {
        ...
        try {
            ...
            if ($config['session']['create_new_app']) {
                // ReactPHP 模式：每次请求创建新应用实例
                $response = $this->handleRequestWithNewApp($request);
            } else {
                // 优化模式：使用现有应用实例但修复 session
                $response = $this->handleWorkermanRequestFixed($request);
            }
    ...
    /**
     * 使用新应用实例处理请求（参考 ReactPHP adapter）
     *
     * @param Request $request
     * @return Response
     */
    protected function handleRequestWithNewApp(Request $request): Response
    {
        $config = array_merge($this->defaultConfig, $this->config);
        ...
        try {
            ...
            // 恢复原始全局变量，但智能合并 cookie，保留 session cookie
            $_GET = $originalGet;
            $_POST = $originalPost;
            $_FILES = $originalFiles;

            // 关键修复：智能合并 cookie，保留 session cookie
    ...
    /**
     * 智能合并 session cookie，保留 session 相关的 cookie（参考 ReactPHP adapter）
     *
    ...
    protected function mergeSessionCookies(array $originalCookies, array $currentCookies): array
    {
        ...
        $sessionCookieNames = [
            ...
            'yii_session',         // Yii session cookie
        ];

        // 从原始 cookies 开始
        $mergedCookies = $originalCookies;

        // 合并 session 相关的 cookie
        foreach ($sessionCookieNames as $sessionName) {
            if (isset($currentCookies[$sessionName])) {
                $mergedCookies[$sessionName] = $currentCookies[$sessionName];
            }
        }

        // 检查是否有其他以 session 相关关键词开头的 cookie
        foreach ($currentCookies as $name => $value) {
            $lowerName = strtolower($name);
            if (str_contains($lowerName, 'session') ||
                str_contains($lowerName, 'sess') ||
                str_contains($lowerName, 'auth') ||
                str_contains($lowerName, 'login') ||
    ...
    protected function handleWorkermanRequestFixed(Request $request): Response
    {
        ...
        if ($config['session']['preserve_session_cookies']) {
            ...
            // 关键修复：正确处理 session
            $this->initializeSessionForRequest($config);

            // 处理请求 - 直接使用 Workerman 请求处理
            $response = $this->handleWorkermanDirectRequest($request);

            // 关键修复：保存 session
            $this->finalizeSessionForRequest($config);

            return $response;

        } finally {
            // 恢复原始全局变量，但智能合并 cookie
            $_GET = $originalGet;
            $_POST = $originalPost;
            $_FILES = $originalFiles;
     ...
     * @return void
     */
    protected function preprocessRequest(Request $request): void
    {
        $config = array_merge($this->defaultConfig, $this->config);
    ...
    protected function handleWorkermanError(Throwable $e): Response
    {
        $content = json_encode([
            ...
            'file' => $e->getFile(),
            'line' => $e->getLine(),
            'adapter' => 'workerman-session-fixed',
    ...
    protected function createStatusResponse(Request $request): Response
    {
        ...
        if ($isBrowserRequest) {
            ...
            $headers = [
                ...
                'Server' => 'Workerman-ThinkPHP-Runtime-SessionFixed',
                'X-Powered-By' => 'ThinkPHP-Runtime/Workerman-SessionFixed',
            ];
        } else {
            // 返回 JSON 响应给 API 客户端
            $data = [
                'message' => 'Workerman Runtime Status (Session Fixed)',
                'path' => $path,
                'method' => $method,
                'timestamp' => time(),
    ...
    protected function createHtmlResponse(string $path, string $method): string
    {
        ...
        $data = [
            'message' => 'Hello from Workerman Runtime (Session Fixed)!',
            'path' => $path,
            'method' => $method,
            'timestamp' => time(),
            'server' => 'Workerman/' . Worker::VERSION,
            'php_version' => PHP_VERSION,
            'memory_usage' => round(memory_get_usage(true) / 1024 / 1024, 2) . 'MB',
            'session_fix_enabled' => $config['session']['enable_fix'],
            'new_app_per_request' => $config['session']['create_new_app'],
            'preserve_session_cookies' => $config['session']['preserve_session_cookies'],
        ];
...
<head>
    ...
    <title>Workerman Runtime - ThinkPHP (Session Fixed)</title>
    <style>
        ...
        .stat-item { background: #f8f9fa; padding: 15px; border-radius: 5px; border-left: 4px solid #28a745; }
        ...
        .fix-info { background: #fff3cd; padding: 15px; border-radius: 5px; margin: 20px 0; border-left: 4px solid #ffc107; }
    </style>
</head>
<body>
    <div class="container">
        <h1>🚀 Workerman Runtime (Session Fixed)</h1>

        <div class="info">
            <strong class="success">✅ Workerman Runtime 运行正常！</strong><br>
            这是一个基于 Workerman 的高性能 ThinkPHP 运行时环境，已修复 session_id 变化问题。
        </div>

        <div class="fix-info">
            <strong>🔧 Session 修复状态:</strong><br>
            • Session 修复: ' . ($data['session_fix_enabled'] ? '<span class="success">已启用</span>' : '未启用') . '<br>
            • 新应用实例模式: ' . ($data['new_app_per_request'] ? '<span class="success">已启用</span>' : '未启用') . '<br>
            • Session Cookie 保留: ' . ($data['preserve_session_cookies'] ? '<span class="success">已启用</span>' : '未启用') . '
        </div>
        ...
        <h2>🔧 Session 修复功能</h2>
        <ul>
            <li>✅ <strong>智能 Cookie 合并</strong>：保留 session 相关的 cookie</li>
            <li>✅ <strong>ReactPHP 模式</strong>：可选的新应用实例创建模式</li>
            <li>✅ <strong>Session 调试</strong>：可启用详细的 session 处理日志</li>
            <li>✅ <strong>向后兼容</strong>：保持原有性能优化</li>
        </ul>
...
$config = [
    "session" => [
        "enable_fix" => true,           // 启用 session 修复
        "create_new_app" => false,      // ReactPHP 模式（性能较低但更安全）
        "preserve_session_cookies" => true, // 保留 session cookie
        "debug_session" => false,       // 调试模式
    ],
];
        </pre>

        <div class="footer">
            <p>Powered by <strong>ThinkPHP Runtime</strong> + <strong>Workerman</strong> (Session Fixed)</p>
            <p>高性能 PHP 应用运行时环境 - Session 问题已修复</p>
        </div>
    </div>
</body>
</html>';
    ...
    public function getMemoryStats(): array
    {
        return array_merge($this->memoryStats, [
            ...
            'session_fix_enabled' => $this->getConfig()['session']['enable_fix'] ?? true,
        ]);
    }
     ...
     * @return void
     */
    protected function initializeSessionForRequest(array $config): void
    {
        if (!$config['session']['enable_fix']) {
            return;
        }
     ...
     * @return void
     */
    protected function finalizeSessionForRequest(array $config): void
    {
        if (!$config['session']['enable_fix']) {
            return;
        }
    ...
    protected function handleSessionWithoutStart(array $config): void
    {
        ...
        try {
            ...
            // 直接从文件读取 session 数据
            $sessionData = $this->readSessionFromFile($sessionId);
     ...
     * @return array|false
     */
    protected function readSessionFromFile(string $sessionId)
    {
        try {
     ...
     * @return bool
     */
    protected function writeSessionToFile(string $sessionId, array $sessionData): bool
    {
        try {
            $sessionPath = session_save_path() ?: sys_get_temp_dir();
            $sessionFile = $sessionPath . '/sess_' . $sessionId;
            
            // 设置 $_SESSION 以便 session_encode 工作
